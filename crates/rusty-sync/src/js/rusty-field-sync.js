/**
 * rusty-field-sync.js - WebSocket-based Field-Level Sync
 * Client-side field-level synchronization for Rusty with WebSocket/SSE support
 *
 * Features:
...
(function() {
    ...
    class RustyFieldSync {
        constructor(config) {
            ...
            this.log('Initializing Rusty Field Sync', {
        ...
        log(...args) {
            if (this.debug) {
                console.log('[Rusty Field Sync]', ...args);
            }
        }

        error(...args) {
            console.error('[Rusty Field Sync]', ...args);
        }
        ...
        async initIndexedDB() {
            return new Promise((resolve, reject) => {
                const request = indexedDB.open('rusty-field-cache', 2);
        ...
        updateConnectionState(newState) {
            ...
            if (oldState !== newState) {
                ...
                window.dispatchEvent(new CustomEvent('rusty:field:connection:state', {
        ...
        handleConflict(conflict) {
            ...
            // Emit custom event
            window.dispatchEvent(new CustomEvent('rusty:field:conflict', {
                detail: conflict
            }));
        ...
        triggerRefresh(entity) {
            const event = new CustomEvent(`rusty:${entity}:field-changed`, {
        ...
        setupBroadcastChannel() {
            ...
            try {
                this.broadcastChannel = new BroadcastChannel('rusty-field-sync');
        ...
        static async init() {
            ...
            if (!entities) {
                console.error('[Rusty Field Sync] No entities specified in data-sync-entities');
                return;
            }
            ...
            const sync = new RustyFieldSync(config);
            ...
            try {
                ...
                window.RustyFieldSync = sync;

                console.log('[Rusty Field Sync] Initialization complete');
                window.dispatchEvent(new CustomEvent('rusty:field:sync:ready'));

            } catch (error) {
                console.error('[Rusty Field Sync] Initialization failed:', error);
            }
        }
    }
    ...
    if (document.currentScript && document.currentScript.hasAttribute('data-sync-entities')) {
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', () => RustyFieldSync.init());
        } else {
            RustyFieldSync.init();
        }
    }
    ...
    window.RustyFieldSync = RustyFieldSync;
})();
