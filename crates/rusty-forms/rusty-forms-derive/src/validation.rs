// File: rusty-forms-derive/src/validation.rs
    ...
    for attr in attrs {
        ...
        match attr_name.as_deref() {
            ...
            Some("depends_on") => {
                ...
                if params.len() >= 2 {
                    validations.push(ValidationAttr::DependsOn(
                        params[0].clone(),
                        params[1].clone(),
                    ));
                }
...
pub fn impl_validate(input: &DeriveInput) -> TokenStream {
    ...
    for field in fields {
        ...
        let custom_message = validations.iter().find_map(|v| match v {
        ...
        for validation in &validations {
            ...
            let validation_check = match validation {
                ValidationAttr::Email => {
                    quote! {
                        if !rusty_forms_validation::is_valid_email(&self.#field_name) {
                ...
                ValidationAttr::NoPublicDomains => {
                    quote! {
                        if rusty_forms_validation::is_public_domain(&self.#field_name) {
                            errors.entry(#field_name_str.to_string())
                                .or_insert_with(Vec::new)
                ...
                ValidationAttr::BlockedDomains(domains) => {
                    let domains_vec = domains
                        ...
                        .collect::<Vec<_>>();
                    quote! {
                        if rusty_forms_validation::is_blocked_domain(&self.#field_name, &vec![#(#domains_vec),*]) {
                ...
                ValidationAttr::Password(pattern) => {
                    quote! {
                        if let Err(msg) = rusty_forms_validation::validate_password(&self.#field_name, #pattern) {
                ...
                ValidationAttr::Regex(pattern) => {
                    quote! {
                        if !rusty_forms_validation::matches_regex(&self.#field_name, #pattern) {
                    ...
                    if is_option {
                        quote! {
                            if let Some(ref value) = self.#field_name {
                                if !rusty_forms_validation::is_valid_url(value) {
                        ...
                        quote! {
                            if !rusty_forms_validation::is_valid_url(&self.#field_name) {
                                errors.entry(#field_name_str.to_string()).or_insert_with(Vec::new).push("Invalid URL".to_string());
                            }
                        }
                    }
                ...
                ValidationAttr::DependsOn(dep_field, dep_value) => {
                    let dep_field_ident =
                        syn::Ident::new(dep_field, proc_macro2::Span::call_site());
                    quote! {
        ...
        if !is_option && !has_allow_whitespace {
            ...
            if is_string_type(&field.ty) {
                let error_msg =
                    custom_message.unwrap_or_else(|| format!("{} is required", field_label));
    ...
    quote! {
        impl rusty_forms_validation::Validate for #name {
...
pub fn impl_form_field(input: &DeriveInput) -> TokenStream {
    ...
    for field in fields {
        ...
        field_match_arms.push(quote! {
            #field_name_str => {
                ...
                rusty_forms_validation::FieldAttrs {
                    html5_attrs: attrs,
    ...
    let field_names_array: Vec<_> = field_names_list
        .iter()
        .map(|name| quote! { #name })
        .collect();

    quote! {
        impl rusty_forms_validation::FormField for #name {
            fn field_attrs(&self, field_name: &str) -> rusty_forms_validation::FieldAttrs {
                match field_name {
                    #(#field_match_arms)*
                    _ => rusty_forms_validation::FieldAttrs::default(),
                }
            }
